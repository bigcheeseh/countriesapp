...
export const FavoriteCountries = React.createContext<{
  countryCodes: string[];
  setCountryCode?(code: string): () => void;
}>({ countryCodes: [], setCountryCode: undefined });

const FavoriteCountriesWrapper = (props: Props) => {
  const [favoriteCountryCodes, setFavoriteCountryCode] = useState<
    Country["alpha2Code"][]
  >([]);
  ...
  useEffect(() => {
    AsyncStorage.setItem(
      "favoriteCountryCodes",
      JSON.stringify(favoriteCountryCodes),
    );
  }, [favoriteCountryCodes]);
  ...
  return (
    <FavoriteCountries.Provider
      value={{
        ...
        setCountryCode: handleSetFavoriteCountryCode,
      }}>
      {props.children}
    </FavoriteCountries.Provider>
  );
